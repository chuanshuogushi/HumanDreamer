...
class ResnetBlock2D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        conv_cls = nn.Conv1d if USE_PEFT_BACKEND else LoRACompatibleConv
...
class SpatioTemporalResBlock(nn.Module):
    ...
    def forward(
        ...
        batch_frames, channels, num_points = hidden_states.shape
        batch_size = batch_frames // num_frames

        hidden_states_mix = (
            hidden_states[None, :].reshape(batch_size, num_frames, channels, num_points).permute(0, 2, 1, 3)
        )
        hidden_states = (
            hidden_states[None, :].reshape(batch_size, num_frames, channels, num_points).permute(0, 2, 1, 3)
        )
        ...
        hidden_states = hidden_states.permute(0, 2, 1, 3).reshape(batch_frames, channels, num_points)
        return hidden_states
...
class AlphaBlender(nn.Module):
    ...
    def get_alpha(self, image_only_indicator: torch.Tensor, ndims: int) -> torch.Tensor:
        if self.merge_strategy == "fixed":
            ...
            # (batch, channel, frames, numpoints)
            if ndims == 4:
                alpha = alpha[:, None, :, None]
            # (batch*frames, numpoints, channels)
            elif ndims == 3:
                alpha = alpha.reshape(-1)[:, None, None]
            else:
