...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            hidden_states = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        elif input_ndim == 3 and type=='vae':  # num points # vae1d时输入的是[b,c,n]，所以需要转置，但是对应unet时输入的是[b,n,c]，不需要转置
            batch_size, channel, _ = hidden_states.shape  # vae1d-unet:[64,16,320]!!!不符合啊
            hidden_states = hidden_states.transpose(1, 2)
        # 从这里往下，hidden_states should be [b,n,c]
        ...
        if input_ndim == 4:
            hidden_states = hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)
        elif input_ndim == 3 and type=='vae':
            hidden_states = hidden_states.transpose(1, 2)
        if attn.residual_connection:
            hidden_states = hidden_states + residual
