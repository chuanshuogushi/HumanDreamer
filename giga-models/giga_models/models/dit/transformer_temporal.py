...
try:
    ...
    from diffusers.models import transformer_temporal
except:
    ...
    def forward(
        ...
        for block, temporal_block in zip(self.transformer_blocks, self.temporal_transformer_blocks):
            if self.training and self.gradient_checkpointing:
                hidden_states = torch.utils.checkpoint.checkpoint(
                    block,
                    hidden_states,  # vae2d-unet:[64,16,320]
                    None,
...
class TransformerSpatioTemporalModel1D(nn.Module):
    """
    A Transformer model for video-like data.
    ...
    def forward(
        ...
        # 1. Input
        # batch_frames, _, height, width = hidden_states.shape
        batch_frames, _, numpoints = hidden_states.shape  # [64,320,16]
        num_frames = image_only_indicator.shape[-1]
        batch_size = batch_frames // num_frames
        ...
        # )
        time_context = time_context_first_timestep[None, :].broadcast_to(
            numpoints, batch_size, 1, time_context.shape[-1]
        )        
        # time_context = time_context.reshape(height * width * batch_size, 1, time_context.shape[-1])
        time_context = time_context.reshape(numpoints * batch_size, 1, time_context.shape[-1])
        ...
        inner_dim = hidden_states.shape[1]
        # hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(batch_frames, height * width, inner_dim)
        hidden_states = hidden_states.permute(0, 2, 1).reshape(batch_frames, numpoints, inner_dim)  # [64,16,320]
        ...
        hidden_states = self.proj_out(hidden_states)
        # hidden_states = hidden_states.reshape(batch_frames, height, width, inner_dim).permute(0, 3, 1, 2).contiguous()
        hidden_states = hidden_states.reshape(batch_frames, numpoints, inner_dim).permute(0, 2, 1).contiguous()
